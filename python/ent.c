...
static int
libuser_entity_setattr(struct libuser_entity *self, char *name,
		       PyObject * args)
{
	PyObject *list;
	int size, i;
	...
	if (PyArg_ParseTuple(args, "O", &list)) {
		...
		if (PyList_Check(list)) {
			...
			lu_ent_set(self->ent, name,
				   PyString_AsString(list));
			DEBUG_EXIT;
			return 0;
		}
	}
...
libuser_entity_getattrlist(struct libuser_entity *self, PyObject * args)
{
	DEBUG_CALL;
	return
	    convert_glist_pystringlist(lu_ent_get_attributes(self->ent));
}
...
libuser_entity_get(struct libuser_entity *self, PyObject * args)
{
	...
	if (lu_ent_has(self->ent, arg)) {
		DEBUG_EXIT;
		return
		    convert_glist_pystringlist(lu_ent_get(self->ent, arg));
	} else {
