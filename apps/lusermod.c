...
main(int argc, const char **argv)
{
	const char *userPassword = NULL, *cryptedUserPassword = NULL,
	    *uid = NULL, *old_uid = NULL, *user = NULL, *gecos =
	    NULL, *oldHomeDirectory, *homeDirectory = NULL, *loginShell =
	    NULL;
	...
	GList *values, *i;
	int change = FALSE, move_home = FALSE, lock = FALSE, unlock =
	    FALSE;
	...
	if (userPassword) {
		if (lu_user_setpass(ctx, ent, userPassword, &error) ==
		    FALSE) {
			fprintf(stderr,
				_
	...
	if (change && old_uid && uid) {
		...
		values =
		    lu_groups_enumerate_by_user(ctx, old_uid, NULL,
						&error);
		if (error) {
			lu_error_free(&error);
		}
	...
	if (change && move_home) {
		if (oldHomeDirectory == NULL) {
			fprintf(stderr,
				_("No old home directory for %s.\n"),
				user);
			return 10;
		}
		if (homeDirectory == NULL) {
			fprintf(stderr,
				_("No new home directory for %s.\n"),
				user);
			return 11;
		}
		if (lu_homedir_move
		    (oldHomeDirectory, homeDirectory, &error) == FALSE) {
