...
main(int argc, const char **argv)
{
	...
	struct poptOption options[] = {
		...
		{"file", 'f', POPT_ARG_STRING, &file, 0, "file with user information records", "STDIN"},
		{"nocreatehome", 'M', POPT_ARG_NONE, &nocreatehome, 0, "don't create home directories", NULL},
		POPT_AUTOHELP
	...
	while(fgets(buf, sizeof(buf), fp)) {
		...
		if(fields && fields[0] && fields[1] && fields[2] && fields[3] && fields[4] && fields[5] && fields[6]) {
			...
			if(strlen(fields[5])) {
				...
				lu_ent_set(ent, LU_HOMEDIRECTORY, fields[5]);
			} else {
				GList *values = lu_ent_get(ent, LU_HOMEDIRECTORY);
				if(values) {
					homedir = g_strdup((char*) values->data);
				} else {
					homedir = g_strdup_printf("/home/%s", fields[0]);
				}
			}
			if(strlen(fields[6])) {
			...
			if(lu_user_add(ctx, ent, &error)) {
				...
				g_print(_("Error creating user account for %s: %s\n"), fields[0],
					error ? error->string : _("unknown error"));
				if(error) {
					lu_error_free(&error);
				}
